...
export default class SMSideMenu extends Menu {
  constructor(items = [], options) {
    ...
    if (this.options.overlay) {
      ...
      this.overlay.el.addEventListener(isTouch ? 'touchstart' : pressEvent, this._onCloseProxy)
    }
    ...
    function walkItems(parent, subMenu) {
      if (subMenu) {
        ...
        for (var i in subMenu.items) {
          if (subMenu.items[i] instanceof SMSubMenuItem) walkItems(parent, subMenu.items[i].subMenu)
        }
      }
    }
  ...
  _refresh () {
    if (this._target) {
      let addeds = [].slice.call(this._target.querySelectorAll('.sidemenu-added'))
