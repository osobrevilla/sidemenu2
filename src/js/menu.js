...
export default class Menu {
  ...
  _add(menuItem, index) {
    ...
    if (this._list.hasChildNodes()) {
      if (this._list.childNodes[index]) this._list.insertBefore(menuItem.el, this._list.childNodes.item(index))
  ...
  _hideSubMenus() {
    if (this.items && this.items.length)
      for (let item of this.items)
        if (item instanceof SMSubMenuItem) 
          item.subMenu._closeWithChilds()
  }
  _closeWithChilds() {
    this._hide();
    this._hideSubMenus();
  ...
  open() {
    if (this.isOpen)
      return this;
    var currentMenu = this._getCurrentMenu(), overlay;
  ...
  destroy() {
    ...
    if (this.parentItem) this.parentItem.items.splice(this.parentItem.items.indexOf(this), 1);
  }
}